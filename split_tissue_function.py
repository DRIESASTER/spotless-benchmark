...
def split_tissue(input_pipeline, spatial_coords, n, figure=True, output_dir='output_files'):
    ...
    for part_value in sorted(input_pipeline.obs['part'].unique()):
        part_data = input_pipeline[input_pipeline.obs['part'] == part_value].copy()
        part_data.__dict__['_raw'].__dict__['_var'] = part_data.__dict__['_raw'].__dict__['_var'].rename(columns={'_index': 'features'})  # Needed otherwise there is a conflict
